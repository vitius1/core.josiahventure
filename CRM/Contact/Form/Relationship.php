...
class CRM_Contact_Form_Relationship extends CRM_Core_Form {
  ...
  public function buildQuickForm() {
    ...
    $relationships = CRM_Contact_BAO_Relationship::getContactRelationshipType($this->_contactId, $this->_rtype, $this->_relationshipId);
    ...
    if($type=="relationship") {
      foreach ($relationships as $key => $value) {
        if(strpos($value, 'relationship - ') !== false) {
          $relationshipList[$key]=str_replace('relationship - ', '', $value);
        }
      }
    } elseif($type=="role") {
      foreach ($relationships as $key => $value) {
        if(strpos($value, 'role - ') !== false) {
          $relationshipList[$key]=str_replace('role - ', '', $value);
        }
      }
    } else {
